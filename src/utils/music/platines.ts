...
import GuildSettingJsonFile, {
	queueItem,
} from "../../../database/schema/guildSettings";
...
export class Platines extends EventEmitter {
	...
	updateSettings = (updater: (settings: GuildSettingJsonFile) => void) =>
		updateSettings(this.guild, updater);
	...
	async play(
		ressource: LasidoAudioRessource,
		source: ressourceSource = "streaming_provider"
	) {
		this.currentRessource = {
			ressource,
			source,
		};
		this.currentRessource.ressource.volume?.setVolume(
			(await this.settings).music.options.volume
		);
	...
	async playTrack(id: number) {
		...
		const infos = await fromQueueType(track)
			.then((d) => converter.convertToYoutubeVideos(d))
			.then((r) => r[0]);
		if (!(infos instanceof YouTubeVideo)) {
			console.error(
				`I did not found any youtube video for track with id <${track.src}:${track.id}>.`
			);
			return false;
		}
		...
		getInfosEmbed(infos).then((embed) =>
			this.broadcast({
				content: "Now playing:",
				embeds: [embed],
			})
	...
	get status(): keyof typeof AudioPlayerStatus {
		...
		for (const key in AudioPlayerStatus) {
			if (
				status ===
				AudioPlayerStatus[key as keyof typeof AudioPlayerStatus]
			)
				return key as keyof typeof AudioPlayerStatus;
		}
	...
	async next() {
		const {
			active_track,
			queue,
			options: { loop, shuffle },
		} = (await this.settings).music;
	...
	async addToQueue(
		...
		await this.updateSettings((s) => {
			if (typeof atIndex === "number") {
				if (s.music.active_track >= atIndex) s.music.active_track++;
	...
	on<E extends keyof PlatinesEvents>(
		eventName: E,
		listener: PlatinesEvents[E]
	) {
		return super.on(eventName, listener);
	}
	emit<E extends keyof PlatinesEvents>(
		eventName: E,
		...args: Parameters<PlatinesEvents[E]>
	) {
